...
using static ITRIProject.Controllers.CommonController;

namespace ITRIProject.Controllers
{
    public class OrganizeCSVController : CommonController
    {
        private MyDbContext _MyDbContext;
        public OrganizeCSVController(MyDbContext MyDbContext)
        {
        ...
        public ActionResult EnergyUseImport(int BasicId)
        {
            ...
            catch (Exception ex)
            {
                var Log = new Log()
                {
                    WhereFunction = "OrganizeCSV_EnergyUseImportCSV_get",
...

        {
            ...
            string fileDir = FileSave.Path + "/CSV";//檔案路徑
            ...
            try
            {
                ...
                string filePath = FileSave.Path + "/CSV" + fileName;//檔案路徑
                ...
                CsvImport.CheckCsvImports(filePath);
                var EnergyUsedt = CsvImport.CsvImports(filePath);
                List<string> errorList = new List<string>();
                if (EnergyUsedt.Rows.Count == 0)
                {
                    errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 資料為空");
                }
                //判斷是否有錯誤
                for (int i = 0; i < EnergyUsedt.Rows.Count; i++)
                {
                    var FactoryName = EnergyUsedt.Rows[i]["場域"].ParseToString();
                    var EnergyName = EnergyUsedt.Rows[i]["能源名稱"].ParseToString();
                    var EquipmentName = EnergyUsedt.Rows[i]["錶號或使用設備名稱"].ParseToString();
                    var EquipmentLocation = EnergyUsedt.Rows[i]["錶或設備位置"].ParseToString();
                    var SupplierName = EnergyUsedt.Rows[i]["供應商名稱"].ParseToString();


                    var Method = EnergyUsedt.Rows[i]["運輸方式"].ParseToString();
                    var StartLocation = EnergyUsedt.Rows[i]["起點"].ParseToString();
                    var EndLocation = EnergyUsedt.Rows[i]["迄點"].ParseToString();
                    var Car = EnergyUsedt.Rows[i]["車種"].ParseToString();
                    var Tonnes = EnergyUsedt.Rows[i]["噸數"].ParseToDecimal();
                    var Fuel = EnergyUsedt.Rows[i]["燃料"].ParseToString();
                    var Land = EnergyUsedt.Rows[i]["陸運(km)"].ParseToDecimal(-1);
                    var LandCondition = EnergyUsedt.Rows[i]["陸運(km)"].ParseToDecimal();//陸運條件
                    var Sea = EnergyUsedt.Rows[i]["海運(nm)"].ParseToDecimal(-1);
                    var SeaCondition = EnergyUsedt.Rows[i]["海運(nm)"].ParseToDecimal();//海運條件
                    var Air = EnergyUsedt.Rows[i]["空運(km)"].ParseToDecimal(-1);
                    var AirCondition = EnergyUsedt.Rows[i]["空運(km)"].ParseToDecimal();//空運條件

                    var Management = EnergyUsedt.Rows[i]["管理單位"].ParseToString();
                    var Principal = EnergyUsedt.Rows[i]["負責人員"].ParseToString();
                    var Datasource = EnergyUsedt.Rows[i]["數據來源"].ParseToString();

                    var Remark = EnergyUsedt.Rows[i]["備註"].ParseToString();
                    var Unit = EnergyUsedt.Rows[i]["單位"].ParseToString();
                    var DistributeRatio = EnergyUsedt.Rows[i]["分配比率"].ParseToDecimal(-1);
                    var DistributeRatioCondition = EnergyUsedt.Rows[i]["分配比率"].ParseToDecimal(); //分配比率條件


                    if (!_MyDbContext.BasicdataFactoryaddresses.Any(a => a.BasicId == BasicId && a.Name == FactoryName))
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(場域不存在)");
                    }
                    if (!_MyDbContext.Coefficients.Any(a => a.Name == EnergyName && a.Type == "EnergyName"))
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(能源名稱不存在)");
                    }
                    if (!_MyDbContext.Suppliermanages.Any(a => a.SupplierName == SupplierName && a.Account == AccountId()))
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(供應商名稱不存在)");
                    }
                    if (!_MyDbContext.Selectdata.Any(a => a.Name == Method) && Method != "")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(運輸方式不存在)");
                    }
                    if (EnergyName == "電力" && Method != "管線") //電力只能為管線
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(能源名稱為：電力，運輸方式應為：管線)");
                    }
                    if (!_MyDbContext.Selectdata.Any(a => a.Name == Car) && Car != "")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(車種不存在)");
                    }
                    if (EnergyName == "電力")
                    {
                        if (!_MyDbContext.Selectdata.Any(a => a.Name == Unit && (a.Code == "25" || a.Code == "117")))
                        {
                            errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(電力單位須為:度(kwh),千度(mwh))");
                        }
                    }
                    if (EnergyName == "車用柴油" || EnergyName == "車用汽油" || EnergyName == "柴油" || EnergyName == "汽油" || EnergyName == "液化石油氣")
                    {
                        ...
                        {
                            errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(車用柴油須為:公升(L)),公秉(kl))");
                        }
                        else if (EnergyName == "車用汽油" && !_MyDbContext.Selectdata.Any(a => a.Name == Unit && (a.Code == "14" || a.Code == "15")))
                        {
                            errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(車用汽油須為:公升(L)),公秉(kl))");
                        }
                        else if (EnergyName == "柴油" && !_MyDbContext.Selectdata.Any(a => a.Name == Unit && (a.Code == "14" || a.Code == "15")))
                        {
                            errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(柴油須為:公升(L)),公秉(kl))");
                        }
                        else if (EnergyName == "汽油" && !_MyDbContext.Selectdata.Any(a => a.Name == Unit && (a.Code == "14" || a.Code == "15")))
                        {
                            errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(汽油須為:公升(L)),公秉(kl))");
                        }
                        else if (EnergyName == "液化石油氣" && !_MyDbContext.Selectdata.Any(a => a.Name == Unit && (a.Code == "14" || a.Code == "15")))
                        {
                            errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(液化石油氣須為:公升(L)),公秉(kl))");
                        }
                    }
                    if (EnergyName == "天然氣")
                    {
                        ...
                        {
                            errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(天然氣單位須為:立方公尺(m3))");
                        }
                    }
                    if (Land == -1)
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(陸運填寫不正確)");
                    }
                    if (LandCondition < 0)
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(陸運不可小於0)");
                    }
                    if (Sea == -1)
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(海運填寫不正確)");
                    }
                    if (SeaCondition < 0)
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(海運不可小於0)");
                    }
                    if (Air == -1)
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(空運填寫不正確)");
                    }
                    if (AirCondition < 0)
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(空運不可小於0)");
                    }                    
                    if (DistributeRatio == -1 || DistributeRatio == null || DistributeRatioCondition < 0 || DistributeRatioCondition > 100)
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(分配比率應介於0-100之間)");
                    }
                    if (Management == "")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(管理單位為必填項目)");
                    }
                    if (Principal == "")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(負責人員為必填項目)");
                    }
                    if (Datasource == "")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(數據來源為必填項目)");
                    }
                    ...
                    for (int j = 0; j < monthcount + 1; j += SelMonth)
                    {
                        ...
                        var StartTime = EnergyUsedt.Rows[i][year + "/" + month + "起始日期"].ParseToTWDate("1");
                        var EndTime = EnergyUsedt.Rows[i][year + "/" + month + "結束日期"].ParseToTWDate("1");
                        var Num = EnergyUsedt.Rows[i][year + "/" + month + "數值"].ParseToDecimal(-1);
                        var NumCondition = EnergyUsedt.Rows[i][year + "/" + month + "數值"].ParseToDecimal();//數值條件

                        if (StartTime == "1")
                        {
                            errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(" + year + "/" + month + "起始日期" + "填寫不正確)");
                        }
                        if (EndTime == "1")
                        {
                            errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(" + year + "/" + month + "結束日期" + "填寫不正確)");
                        }
                        if (Num == -1)
                        {
                            errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(" + year + "/" + month + "數值" + "填寫不正確)");
                        }
                        if (NumCondition < 0)
                        {
                            errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(" + year + "/" + month + "數值" + "不可為負數)");
                        }
                        if (Num != null &&(StartTime == "" || EndTime == ""))
                        {
                            errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(" + year + "/" + month + "缺少起始或結束時間)");
                        }
                        month += SelMonth;
                        if (month > 12)
                        {
                ...
                {
                    result.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} 資料正確 匯入成功 共匯入{EnergyUsedt.Rows.Count}筆");
                }
            ...
            catch (Exception ex)
            {
                ...
                var Log = new Log()
                {
                    WhereFunction = "OrganizeCSV_EnergyUseImportCSV_post",
                    Exception = ex.ToString(),
                    DateTime = DateTime.Now,
                };
                _MyDbContext.Logs.Add(Log);
        ...
        public ActionResult ResourceUseImport(int BasicId)
        {
            ...
            catch (Exception ex)
            {
                var Log = new Log()
                {
                    WhereFunction = "OrganizeCSV_ResourceUseImportCSV_get",
                    Exception = ex.ToString(),
                    DateTime = DateTime.Now,
                };
                _MyDbContext.Logs.Add(Log);
...

        {
            ...
            try
            {
                ...
                CsvImport.CheckCsvImports(filePath);
                var ResourceUsedt = CsvImport.CsvImports(filePath);
                List<string> errorList = new List<string>();
                ...
                for (int i = 0; i < ResourceUsedt.Rows.Count; i++)
                {
                    ...
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(資源名稱不存在)");
                    }
                    if (!_MyDbContext.Suppliermanages.Any(a => a.SupplierName == SupplierName && a.Account == AccountId()) )
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(供應商名稱不存在)");
                    ...
                    { 
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(單位須為：{UnitCompare[0]})");
                    }
                    if (Land == -1)
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(陸運填寫不正確)");
            ...
            catch (Exception ex)
            {
                ...
                var Log = new Log()
                {
                    WhereFunction = "OrganizeCSV_ResourceUseImportCSV_post",
                    Exception = ex.ToString(),
                    DateTime = DateTime.Now,
                };
                _MyDbContext.Logs.Add(Log);
        ...
        public ActionResult RefrigerantNoneImport(int BasicId)
        {
            ...
            catch (Exception ex)
            {
                var Log = new Log()
                {
                    WhereFunction = "OrganizeCSV_RefrigerantNoneImportCSV_get",
                    Exception = ex.ToString(),
                    DateTime = DateTime.Now,
                };
                _MyDbContext.Logs.Add(Log);
...

        {
            ...
            try
            {
                ...
                CsvImport.CheckCsvImports(filePath);
                var RefrigerantNonedt = CsvImport.CsvImports(filePath);
                List<string> errorList = new List<string>();
                ...
                for (int i = 0; i < RefrigerantNonedt.Rows.Count; i++)
                {
                    ...
                    if (EquipmentName == "")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(設備名稱為必填項目)");
                    }
                    if (!_MyDbContext.Selectdata.Any(a => a.Name == EquipmentType) )
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(設備類型不存在)");
                    }
                    if (!_MyDbContext.Coefficients.Any(a => a.Name == RefrigerantType && a.Type== "RefrigerantName") )
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(冷媒種類不存在)");
                    }
                    if (MachineQuantity == -1 || MachineQuantity == null|| MachineQuantityCondition < 0)
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(設備數量須為正整數)");
                    }
                    if (Management == "")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(管理單位為必填項目)");
                    ...
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(冷媒重量須為大於0的數值)");
                    }
                    if (Unit != "公斤(kg)")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(單位須為公斤(kg))");
                    }
                    if (DistributeRatio == -1 || DistributeRatio == null || DistributeRatioCondition < 0 || DistributeRatioCondition > 100)
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(分配比率應介於0-100之間)");
            ...
            catch (Exception ex)
            {
                ...
                var Log = new Log()
                {
                    WhereFunction = "OrganizeCSV_RefrigerantNoneImportCSV_post",
                    Exception = ex.ToString(),
                    DateTime = DateTime.Now,
                };
                _MyDbContext.Logs.Add(Log);
        ...
        public ActionResult RefrigerantHaveImport(int BasicId)
        {
            ...
            catch (Exception ex)
            {
                var Log = new Log()
                {
                    WhereFunction = "OrganizeCSV_RefrigerantHaveImportCSV_get",
                    Exception = ex.ToString(),
                    DateTime = DateTime.Now,
                };
                _MyDbContext.Logs.Add(Log);
...

        {
            ...
            try
            {
                ...
                CsvImport.CheckCsvImports(filePath);
                var RefrigerantHavedt = CsvImport.CsvImports(filePath);
                List<string> errorList = new List<string>();
                ...
                for (int i = 0; i < RefrigerantHavedt.Rows.Count; i++)
                {
                    ...
                    var Datasource = RefrigerantHavedt.Rows[i]["數據來源"].ParseToString();
                    ...
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(填充量須為大於0的數)");
                    }
                    if (Unit != "公斤(kg)")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(單位須為公斤(kg))");
            ...
            catch (Exception ex)
            {
                ...
                var Log = new Log()
                {
                    WhereFunction = "OrganizeCSV_RefrigerantHaveImportCSV_post",
                    Exception = ex.ToString(),
                    DateTime = DateTime.Now,
                };
                _MyDbContext.Logs.Add(Log);
        ...
        public ActionResult FireequipmentImport(int BasicId)
        {
            ...
            catch (Exception ex)
            {
                var Log = new Log()
                {
                    WhereFunction = "OrganizeCSV_FireequipmentImportCSV_get",
                    Exception = ex.ToString(),
                    DateTime = DateTime.Now,
                };
                _MyDbContext.Logs.Add(Log);
...

        {
            ...
            try
            {
                ...
                CsvImport.CheckCsvImports(filePath);
                var Fireequipmentdt = CsvImport.CsvImports(filePath);
                List<string> errorList = new List<string>();
                ...
                for (int i = 0; i < Fireequipmentdt.Rows.Count; i++)
                {
                    ...
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(設備名稱不存在)");
                    }
                    if (!_MyDbContext.Selectdata.Any(a => a.Name == Ghgtype && a.Type == "GHGtype"))
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(溫室氣體種類不存在)");
                    }
                    if (!_MyDbContext.Selectdata.Where(a => a.Code == "10").Select(a => a.Name).Contains(Unit))
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(單位須為:公斤(kg)");
                    }
                    if (DistributeRatio == -1 || DistributeRatio == null || DistributeRatioCondition < 0 || DistributeRatioCondition > 100)
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(分配比率應介於0-100之間)");
            ...
            catch (Exception ex)
            {
                ...
                var Log = new Log()
                {
                    WhereFunction = "OrganizeCSV_FireequipmentImportCSV_post",
                    Exception = ex.ToString(),
                    DateTime = DateTime.Now,
                };
                _MyDbContext.Logs.Add(Log);
        ...
        public ActionResult WorkinghourImport(int BasicId)
        {
            ...
            catch (Exception ex)
            {
                var Log = new Log()
                {
                    WhereFunction = "OrganizeCSV_WorkinghourImportCSV_get",
                    Exception = ex.ToString(),
                    DateTime = DateTime.Now,
                };
                _MyDbContext.Logs.Add(Log);
...

        {
            ...
            try
            {
                ...
                CsvImport.CheckCsvImports(filePath);
                var Workinghourdt = CsvImport.CsvImports(filePath);
                List<string> errorList = new List<string>();
                ...
                for (int i = 0; i < Workinghourdt.Rows.Count; i++)
                {
                    ...
                    if (Item == "")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(項目為必填)");
                    }
                    if (TotalWorkHour == null && Item == "正式員工及約聘人員(具勞保)")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(總工時為必填)");
                    }
                    if (Management == "" && Item == "正式員工及約聘人員(具勞保)")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(管理部門為必填)");
                    }
                    if (Principal == "" && Item == "正式員工及約聘人員(具勞保)")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(負責人員為必填)");
                    }
                    if (Datasource == "" && Item == "正式員工及約聘人員(具勞保)")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(數據來源為必填)");
                    }
                    if (DistributeRatio == null && Item == "正式員工及約聘人員(具勞保)")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(分配比率為必填)");
                    }
                    if (Item == "0")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(項目不存在)");
                    }
                    if (TotalWorkHour == -1)
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(總工時填寫須為小數點 2 位以內的數字)");
                    }                    
                    if ((TotalWorkHourCondition < 0 ) && (Item == "正式員工及約聘人員(具勞保)" || Item == "廠商外派人員(不具勞保)")) // 總工時要大於0
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(總工時需大於0)");
                    }
                    if (DistributeRatio == -1)
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(分配比例填寫須為小數點 4 位以內的數字)");
                    }
                }
                //沒錯誤則新增資料
                if (errorList.Count == 0)
            ...
            catch (Exception ex)
            {
                ...
                var Log = new Log()
                {
                    WhereFunction = "OrganizeCSV_WorkinghourImportCSV_post",
                    Exception = ex.ToString(),
                    DateTime = DateTime.Now,
                };
                _MyDbContext.Logs.Add(Log);
        ...
        public ActionResult DumptreatmentOutsourcingImport(int BasicId)
        {
            ...
            catch (Exception ex)
            {
                var Log = new Log()
                {
                    WhereFunction = "OrganizeCSV_DumptreatmentOutsourcingImportCSV_get",
                    Exception = ex.ToString(),
                    DateTime = DateTime.Now,
                };
                _MyDbContext.Logs.Add(Log);
...

        {
            ...
            try
            {
                ...
                CsvImport.CheckCsvImports(filePath);
                var DumptreatmentOutsourcingdt = CsvImport.CsvImports(filePath);
                List<string> errorList = new List<string>();
                ...
                for (int i = 0; i < DumptreatmentOutsourcingdt.Rows.Count; i++)
                {
                    ...
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(廢棄物名稱或廢棄物處理方式未選取)");
                    }
                    else if(_MyDbContext.Coefficients.FirstOrDefault(a => a.Name == WasteName && a.WasteMethod == WasteMethod && a.Type == "dumptreatment_outsourcing") == null)
                    {
                        ...
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因({WasteName}，請選擇{results})");
                    }
                    if (!_MyDbContext.Selectdata.Any(a => a.Name == Method) && Method != "")
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(運輸方式不存在)");
                    ...
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(轉換前單位不存在)");
                    }
                    if (Land == -1)
                    {
                        errorList.Add($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} {file.FileName} 第{i + 2}行失敗，原因(陸運填寫不正確)");
            ...
            catch (Exception ex)
            {
                ...
                var Log = new Log()
                {
                    WhereFunction = "OrganizeCSV_DumptreatmentOutsourcingImportCSV_post",
                    Exception = ex.ToString(),
                    DateTime = DateTime.Now,
                };
                _MyDbContext.Logs.Add(Log);
